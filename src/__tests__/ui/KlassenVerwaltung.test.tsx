...
const mockedGetTeachers = getTeachers as jest.MockedFunction<typeof getTeachers>
...
describe('SchulklassenVerwaltung Page', () => {
    beforeEach(() => {
        ...
        mockedGetClasses.mockResolvedValue([
            { id: 1, name: 'Test Class 1', year: '1-3', track: 'A', primary_teacher_id: 1, secondary_teacher_id: null },
            { id: 2, name: 'Test Class 2', year: '4-6', track: 'B', primary_teacher_id: 2, secondary_teacher_id: null }
        ])
        mockedGetTeachers.mockResolvedValue([
            { id: 1, first_name: 'John', last_name: 'Doe', email: 'john@test.com', phone: '1234567890', priority: 1, weekly_capacity: 40 },
            { id: 2, first_name: 'Jane', last_name: 'Smith', email: 'jane@test.com', phone: '0987654321', priority: 1, weekly_capacity: 40 }
        ])
    })
    ...
    it('renders the page title', async () => {
        render(<SchulklassenVerwaltung />)
        expect(await screen.findByText('Schulklassen-Verwaltung')).toBeInTheDocument()
    })
    ...
    it('shows "Klasse hinzufügen" button and opens dialog on click', async () => {
        ...
        expect(await screen.findByText('Neue Klasse hinzufügen')).toBeInTheDocument()
    })

    it('renders the table headers correctly', async () => {
        render(<SchulklassenVerwaltung />)
        const headers = ['Name', 'Jahrgang', 'Zug', 'Klassenlehrer', 'Co-Klassenlehrer', 'Aktionen']
