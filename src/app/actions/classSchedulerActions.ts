...
export async function saveSchedule(
  ...
  try {
    ...
    for (const lesson of all_lessons) {
      ...
      try {
        const existing =
          lesson.id && (await lessonRepository.getById(lesson.id));
        if (existing) {
...
export async function loadSchedule(
  timetableId?: number,
): Promise<{ lessons: Lesson[]; ablageLessons: Lesson[] }> {
...
export async function getAvailableTeachersForTimeslot(
  day: string,
  timeslot: number,
): Promise<Teacher[]> {
  const availableTeachers = teacherRepository.getAvailableTeachers(
    day,
    timeslot,
  );
...
export async function getAvailableTeachersForEdit(
  day: string,
  timeslot: number,
): Promise<Teacher[]> {
  const availableTeachers = teacherRepository.getAvailableTeachers(
