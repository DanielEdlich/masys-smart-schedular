...
export const Ablage: React.FC<AblageProps> = ({
  ...
  const [isNewLessonDialogOpen, setIsNewLessonDialogOpen] = React.useState(false);
  ...
  const handleMoveLesson = async (
    ...
    if (toDay === 'Ablage' && toName === 'Ablage') {
      ...
      const reindexedLessons = updatedLessons.map((c, index) => ({ ...c, timeslot: index }));
        ...
        if (isAvailable) {
          if (isBooked) {
            toast({
              ...
              description: "Der Lehrer ist zu dieser Zeit bereits in einer anderen Klasse eingeplant",
              variant: "destructive",
            });
          } else {
