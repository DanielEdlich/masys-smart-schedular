...
const weekdays = ['Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag'] as const
type Weekday = typeof weekdays[number]

const classes = ['Class1', 'Class2', 'Class3', 'Class4', 'Class5', 'Class6', 'Class7', 'Class8', 'Class9', 'Class10'] as const
...
const classTypes = ['Mathematik', 'Deutsch', 'Kunst', 'Englisch', 'Sport', 'Biologie', 'Chemie', 'Physik', 'Geschichte', 'Geographie'] as const
type ClassType = typeof classTypes[number]

const teachers = ['Herr Schmidt', 'Frau Müller', 'Herr Weber', 'Frau Fischer', 'Herr Wagner', 'Frau Becker', 'Herr Hoffmann', 'Frau Schulz', 'Herr Koch', 'Frau Meyer'] as const
type Teacher = typeof teachers[number]

const getColorByTeacher = (teacher: Teacher): string => {
  const colors = [
    '#FF6B6B', '#4ECDC4', '#45B7D1', '#FFA07A', '#98FB98', '#DDA0DD', '#F0E68C', '#87CEFA',
    '#FFB6C1', '#20B2AA'
  ];
...
interface DraggableCardProps {
  card: ScheduleCard
  moveCard: (card: ScheduleCard, toDay: Weekday, toClass: ClassName, toWeek: Week, toIndex: number) => void
  editCard: (day: Weekday, className: ClassName, week: Week, index: number, name: string, type: string, color: string, secondaryTeacher: string) => void
...
interface CardFormProps {
  ...
  week: Week
  slotIndex: number
  addCard: (day: Weekday, className: ClassName, week: Week, index: number, teacher: Teacher, type: ClassType, color: string, secondaryTeacher?: Teacher) => void
  editCard: (day: Weekday, className: ClassName, week: Week, index: number, teacher: Teacher, type: ClassType, color: string, secondaryTeacher?: Teacher) => void
  deleteCard: (day: Weekday, className: ClassName, week: Week, index: number) => void
  existingCard?: ScheduleCard
  onClose: () => void
}

const CardForm: React.FC<CardFormProps> = ({ day, className, week, slotIndex, addCard, editCard, deleteCard, existingCard, onClose }) => {
  const [teacher, setTeacher] = useState<Teacher>(existingCard?.teacher || teachers[0])
  const [type, setType] = useState<ClassType>(existingCard?.type || classTypes[0])
  const [secondaryTeacher, setSecondaryTeacher] = useState<Teacher | 'none'>(existingCard?.secondaryTeacher || 'none')

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    const secondaryTeacherValue = secondaryTeacher === 'none' ? undefined : secondaryTeacher
    const color = getColorByTeacher(teacher)
    if (existingCard) {
      editCard(day, className, week, slotIndex, teacher, type, color, secondaryTeacherValue)
    } else {
      addCard(day, className, week, slotIndex, teacher, type, color, secondaryTeacherValue)
    }
    onClose()
  }
    ...
    <form onSubmit={handleSubmit}>
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Select value={type} onValueChange={(value: ClassType) => setType(value)} required>
            <SelectTrigger>
        ...
        <div className="space-y-2">
          ...
          <Select value={teacher} onValueChange={(value: Teacher) => setTeacher(value)} required>
            <SelectTrigger>
        ...
        <div className="space-y-2">
          ...
          <Select value={secondaryTeacher} onValueChange={(value: Teacher | 'none') => setSecondaryTeacher(value)}>
            <SelectTrigger>
        ...
        <div className="flex justify-between">
          <Button type="submit">{existingCard ? 'Aktualisieren' : 'Hinzufügen'}</Button>
          {existingCard && (
...
const DroppableSlot: React.FC<DroppableSlotProps> = ({ day, className, week, slotIndex, card, moveCard, addCard, editCard, deleteCard }) => {
  const [, drop] = useDrop({
    accept: 'CARD',
    canDrop: () => !isBlocked,
    drop: (item: ScheduleCard) => {
      if (!isBlocked && (item.day !== day || item.className !== className || item.week !== week || item.index !== slotIndex)) {
  ...
  return (
    <div ref={drop} className="w-32 h-16 relative  rounded-md overflow-hidden">
      {isBlocked ? (
        ...
        <DraggableCard card={card} moveCard={moveCard} editCard={editCard} deleteCard={deleteCard} />
      ) : (
...
export default function Component({ initialSchedule }: ClassSchedulerProps = {}) {
  ...
  const addCard = useCallback((day: Weekday, className: ClassName, week: Week, index: number, teacher: Teacher, type: ClassType, color: string, secondaryTeacher?: Teacher) => {
    setSchedule(prevSchedule => {
  ...
  const editCard = useCallback((day: Weekday, className: ClassName, week: Week, index: number, teacher: Teacher, type: ClassType, color: string, secondaryTeacher?: Teacher) => {
    setSchedule(prevSchedule => {
      const newSchedule = { ...prevSchedule }
  ...
  const deleteCard = useCallback((day: Weekday, className: ClassName, week: Week, index: number) => {
    setSchedule(prevSchedule => {
      const newSchedule = { ...prevSchedule }
                ...
                <thead className="sticky top-0 z-20 bg-white">
                  <tr className="sticky top-0 z-20">
                    ...
                    {classes.map(className => (
                      <th key={className} colSpan={2} className="border p-2 bg-purple-50 min-w-[200px]">
                        {className}
                      </th>
                    ))}
                  </tr>
                  <tr className="sticky top-[41px] z-20 bg-white">
                    ...
                    {classes.flatMap(className =>
                      weeks.map(week => (
                        <th key={`${className}-${week}`} className="border p-2 bg-purple-50 min-w-[100px]">
                          Woche {week}
                        </th>
                      ))
                    )}
                  ...
                  {weekdays.map(day => (
                    <React.Fragment key={day}>
                      {timeSlots.map((slot, slotIndex) => (
                        <tr key={`${day}-${slot}`}>
                          {slotIndex === 0 && (
                            <th rowSpan={8} className="border p-2 bg-purple-50 sticky left-0 z-10 min-w-[100px]">
                              {day}
                            </th>
                          )}
                          ...
                          {classes.map(className =>
                            weeks.map(week => (
                              <td key={`${day}-${className}-${week}-${slot}`} className="border p-0.5 min-w-[100px]">
                                <DroppableSlot
                                  day={day}
                                  className={className}
                                  week={week}
                                  slotIndex={slotIndex}
                                  card={schedule[day][className][week][slotIndex]}
                                  moveCard={moveCard}
                                  addCard={addCard}
                                  editCard={editCard}
                                  deleteCard={deleteCard}
